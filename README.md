...
Since the book contains a lot of equations, *normal* text extraction would not yield satisfactory results. To convert the book into LaTeX, run the python script [``rag_training/pdf2latex.py``](rag_training/pdf2latex.py). To use GPU acceleration, update the enviroment variable ``TORCH_DEVICE="cuda"``. For this to work, torch must with compiled with cuda. 
